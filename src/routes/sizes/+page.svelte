<script>
    ...
    import SizesModal from "$lib/modals/SizesModal.svelte";
    ...
    const setFormFieldErrors = (data = {}) => ({
        ...
        category: Boolean(data?.category ?? true)
    })
    ...
    async function loadTableItems(pageOverride = null) {
        ...
        try {
            ...
            if (err.response && err.response.data && err.response.data.message) {
                error = `Error al cargar tallas: ${err.response.data.message}`
            } else {
    ...
    async function handleSubmit(payload){
        ...
        try{
            if(isEditModal){
                const response = await sizesApi.update(editId, payload)
            }else{
                const response = await sizesApi.store(payload);
            } 
            ...
            if (err.response && err.response.data && err.response.data.message) {
                notify.error(`La talla no pudo ser ${message} debido a un error: ${err.response.data.message}`);
            } else {
    ...
    async function handleConfirm(){
        ...
        let message = isToggleModal ? 'Estado cambido exitosamente.' : 'Talla eliminada exitosamente.';
        let errorMessage = isToggleModal ? 'Error al cambiar el estado: ' : 'La talla no pudo ser eliminada debido a un error: '
        modalLoading = true;
        try{
            if(isToggleModal){
                const response = await sizesApi.toggleStatus(toggleId)
            }else{
                const response = await sizesApi.delete(deleteId);
            }
    ...
    function openDeleteModal(id, name){
        ...
        confirmFields = setConfirm({
            ...
            largeMessage: 'Eliminar talla',
            message: `La talla ${name} será eliminada al confirmar ¿Desea continuar?`,
...
{#if can(CURRENT_PERMISSIONS, PERMISSIONS.VIEW_SIZES)}

    <TabHead title='Tallas' />
    <Breadcrumb items={BREADCRUMB_ITEMS} title='Tallas' />
    ...
    <div class='card shadow-sm py-4 px-3 border-0 rounded-4'>
        <PageToolbar 
            ...
            showCreateButton={can(CURRENT_PERMISSIONS, PERMISSIONS.CREATE_SIZES)}
            entityLabel='Tallas'
            perPageId='sizes_per_page' 
            newButtonIcon='bi bi-plus'
            newButtonText='Nueva talla' 
        ...
        >
            {#snippet cell({ row, column, value })}
                ...
                {:else if column.key === 'actions'}
                    {#if can(CURRENT_PERMISSIONS, PERMISSIONS.EDIT_SIZES)}
                        <button
                            title="Editar"
                    ...
                    {/if}
                    {#if can(CURRENT_PERMISSIONS, PERMISSIONS.DELETE_SIZES)}
                        <button
                            title="Borrar"
